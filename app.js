...
function fetchData() {
    apiCall('get_all').then(data => {
        const usersData = data.users || {};
        allUsers = Object.values(usersData);
        if (usersData[currentIP]) {
            currentUser = usersData[currentIP];
            updateProfileUI();
        }

        const msgsData = data.messages || {};
        messages = Object.entries(msgsData).map(([id, val]) => ({ id, ...val }));
        
        const chatsData = data.publicChats || {};
        publicChats = Object.entries(chatsData).map(([id, val]) => ({ id, ...val }));
...
function attachEvents() {
    ...
    document.getElementById('createAccountBtn').addEventListener('click', () => {
        ...
        const newUser = { name: name, email: name + '@satria.dev', ip: currentIP, avatar: avatarUrl, bio: bio, createdAt: Date.now() };
        apiCall('create_user', { ip: currentIP, user: newUser }).then(() => {
            currentUser = newUser;
            fetchData();
            navigate('/home');
        });
    ...
    document.getElementById('clearInboxBtn').addEventListener('click', () => {
        if (confirm('Hapus semua pesan masuk?')) {
            ...
            userMsgs.forEach(m => apiCall('delete_message', { id: m.id }));
    ...
    document.getElementById('sendMsgBtn').addEventListener('click', () => {
        ...
        const msg = { from: currentUser.email, to: targetEmail, text: text, timestamp: Date.now(), read: false };
        apiCall('send_message', { message: msg }).then(() => {
    ...
    document.getElementById('sendMsgBtnV2').addEventListener('click', () => {
        ...
        const msg = { from: senderName, to: targetEmail, text: text, timestamp: Date.now(), read: false };
        apiCall('send_message', { message: msg }).then(() => {
    ...
    document.getElementById('sendPublicBtn').addEventListener('click', () => {
        ...
        if (!txt) return;
        const chat = { from: currentUser.email, senderName: currentUser.name, text: txt, replyTo: replyContext ? { text: replyContext.text, from: replyContext.from, name: replyContext.senderName } : null, timestamp: Date.now() };
        apiCall('send_public', { chat: chat }).then(() => {
            input.value = '';
            replyContext = null;
    ...
    document.getElementById('saveProfileBtn').addEventListener('click', () => {
        ...
        apiCall('update_user', { ip: currentIP, data: { avatar: newUrl, bio: newBio } }).then(() => {
    ...
    document.getElementById('deleteAccountBtn').addEventListener('click', () => {
        if (confirm('Yakin ingin menghapus akun? Semua data akan hilang.')) {
            apiCall('delete_user', { ip: currentIP }).then(() => {
                currentUser = null;
                navigate('/');
            });
        }
...
function renderSendV2() {
    ...
    if(currentUser) {
        senderArea.innerHTML = `<input type="text" class="compose-input" value="${currentUser.email}" disabled style="background:#e5e7eb; color:#4b5563;">`;
    } else {
        senderArea.innerHTML = `
            <label>Nama Pengirim (Kosongkan untuk anonim)</label>
